...
export const isValidEmail = (email) => {
  ...
  if (email.substr(email.length - validEnding.length) === validEnding && email.length > validEnding.length) { return { message: "valid" }; }
  return {
...
export const isValidPhone = (phone) => {
  ...
  if (Number.isInteger(num) && phone.length === 9 && num > 0) { return { message: "valid" }; }
  return { message: "invalid phone", body: "phone must contain 9 digits" };
};

export const isValidDate = (date) => {
  if (date) return { message: "valid" };
  return { message: "invalid date", body: "date must not be empty" };
};
